...
export const weeklyCommand: MinecraftCommand = {
    ...
    run: async (sender, targetIgn, args, uuid) => {
        ...
        const resp = `/msg ${sender} [${stars}✫] ${d.display_name} [WEEKLY]: Finals: ${d.final_kills} * FD: ${d.final_deaths} * FKDR: ${d.fkdr.toFixed(2)} * Wins: ${d.wins} * Losses: ${d.losses} * WLR: ${d.wlr.toFixed(2)} * KDR: ${d.kdr.toFixed(2)} * BBLR: ${d.bblr.toFixed(2)}`;
        mcClient.send(resp);
    }
};
